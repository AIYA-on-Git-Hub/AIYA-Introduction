# .github
# AI Youth Alliance (AIYA) 🌐

Welcome to the official GitHub home of the **AI Youth Alliance (AIYA)**. We are a global, student-led network operating under the auspices of the **Hey AI! Foundation**, dedicated to advancing learning, problem-solving, and critical AI literacy in our schools.

## 🎯 Our Mission
AIYA equips students with the foundational AI fluency, technical techniques, and critical literacy essential for career success and personal well-being. We empower students to exert agency in learning and preserve genuine authorship and analytical capacity in an AI-driven world.

> 🔒 **Official Digital Channel Verification:** The AI Youth Alliance operates exclusively through our official domain: **[aiyouthalliance.org](https://aiyouthalliance.org)**. Any external profiles under similar names using other top-level domains (such as `.tech` or `.com`) are entirely unaffiliated with our network.

---

## 🏛️ Our Academic Forums
We provide structured spaces for safe exploration and discovery. This work is distributed across three student-led, project-based collaborative forums:

*   **💻 AIYA Coding Forum:** Focused on hands-on software development, foundational machine learning algorithms, and programming custom AI agents.
*   **📈 AIYA Finance Forum:** Exploring the intersection of AI, economic modeling, financial analysis, and predictive workflows.
*   **🗳️ AIYA Democracy and Media Forum:** Dedicate to analyzing, problematizing, and collaborating around the impact of AI on media integrity, misinformation, and democratic processes.

---

## 📅 Tentpole Global Initiatives
We bring hands-on experiential AI programming directly into school chapters through three yearly tentpole events:
1.  **AIYA Literacy Week:** Direct, hands-on challenges and problem sets designed to highlight both the vast capabilities and surprising limitations of AI.
2.  **AIYA Global Hackathon:** A first-of-its-kind international competition for building apps, models, and agents using AI.
3.  **AIYA Impact Symposium:** A global showcase for student creative projects, policy whitepapers, and technical research centered around applicaton of artificial intelligence to real-world problems and endeavors.

---

## 🤝 Connect with Us
*   **Official Website:** [aiyouthalliance.org](https://aiyouthalliance.org)
*   **Hugging Face:** [AIYA-on-Huggingface](https://huggingface.co)
*   **Instagram:** [ai.youth.alliance](https://instagram.com)
*   **Youtube:** [@AI_Youth_Alliance](https://youtube.com)
*   **X (Twitter):** [@AIYA_on_X](https://x.com)
*   **LinkedIn:** [AI Youth Alliance (AIYA)](https://linkedin.com)
*   **Contact Email:** info@aiyouthalliance.org

© 2026 AI Youth Alliance & Hey AI! Foundation. All Rights Reserved.
